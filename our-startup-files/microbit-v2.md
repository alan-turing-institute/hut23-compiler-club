...
and `.bss` sections will (eventually) end up in RAM. <sup><a id="fnr.1" class="footref" href="#fn.1" role="doc-backlink">1</a></sup>
...
*somewhere* by the linker anyway. <sup><a id="fnr.2" class="footref" href="#fn.2" role="doc-backlink">2</a></sup>
...
the pattern `*(.text*)` matches all input files (that's
the first asterisk) and, within those, all sections whose name begin
with `.text` (that's the second asterisk). <sup><a id="fnr.3" class="footref" href="#fn.3" role="doc-backlink">3</a></sup>

The interrupt vectors section is wrapped in “`KEEP`”
...
point to the section in `RAM`.” That's what “`>RAM AT >FLASH`” does. <sup><a id="fnr.4" class="footref" href="#fn.4" role="doc-backlink">4</a></sup>

I'm not sure why the two data lines aren't a single line,
`*(.data*)`, but this is what the Arm example linker
...
The `bss` output section merely reserves space (in RAM)
...
-   The [GNU `ld` manual](https://sourceware.org/binutils/docs/ld/index.html) explains the meanings of the various parts of
    the linker script.


# Footnotes

<sup><a id="fn.1" href="#fnr.1">1</a></sup> The example linker script provided by Nordic Semiconductor
...
<sup><a id="fn.2" href="#fnr.2">2</a></sup> We should probably run `ld` with `--orphan-handling=warn`.

<sup><a id="fn.3" href="#fnr.3">3</a></sup> GCC emits multiple text sections when the option
`-ffunction-sections` is used.

<sup><a id="fn.4" href="#fnr.4">4</a></sup> The terminology is as follows. The address of this section at run-time
